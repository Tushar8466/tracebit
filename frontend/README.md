...
generated by an AI — making it nearly impossible for reviewers, educators, and maintainers to
make informed judgments about contribution authenticity.

### Target Users
- 🎓 **Educators & Academic Institutions** — detecting AI-written code in student assignments
  submitted via GitHub
- 🏢 **Recruiters & Hiring Managers** — verifying that coding assessments and portfolio projects
  reflect genuine candidate skills
- 🌐 **Open-Source Maintainers** — ensuring contributions to repositories are human-authored
  and thoughtfully written
- 🤝 **Team Leads & Code Reviewers** — maintaining transparency and accountability in
  collaborative codebases
- 📋 **Compliance & Audit Teams** — organizations requiring human-verified code in regulated
  environments

### Existing Gaps
- ❌ **No Native GitHub Support** — GitHub provides no built-in tooling to detect or label
  AI-generated commits or pull requests
- ❌ **General-purpose detectors fall short** — Tools like GPTZero or Copyleaks are designed
  for natural language text, not code, and perform poorly on programming languages
- ❌ **No commit-level granularity** — Existing solutions (if any) work on file level, not at
  the granularity of individual commits, diffs, or pull requests
- ❌ **No contextual analysis** — Current tools ignore coding style consistency, commit message
  patterns, authorship history, and repository context — all strong signals of AI generation
- ❌ **Lack of integration** — There is no CI/CD-friendly or GitHub Actions-compatible tool that
  can automatically scan contributions during the review pipeline
- ❌ **No explainability** — Even partial solutions provide a binary result with no reasoning,
...
The core issue stems from a **fundamental shift in how code is being written** — AI tools have
...
#### 🔸 1. Invisibility of AI Involvement
AI-generated code looks structurally identical to human-written code on the surface. GitHub's
contribution system only tracks *what* was committed, never *how* it was written — creating a
blind spot in the entire contribution pipeline.

#### 🔸 2. Absence of Behavioral Signals in Reviews
Human developers leave subtle fingerprints — inconsistent variable naming, iterative commit
...
#### 🔸 4. No Domain-Specific Detection Models
Existing AI content detectors are trained on prose and natural language. Code has a fundamentally
different structure — syntax, logic flow, token patterns, and entropy distributions — that generic
...
- **Code-level signals** — token entropy, cyclomatic complexity, comment-to-code ratio,
  naming convention uniformity, code structure patterns
- **Commit-level signals** — commit size, time between commits, message quality and length,
  frequency of single large dumps vs. incremental changes
- **Author-level signals** — deviation from the author's historical coding style and
...
to code — not natural language.

#### 🔹 3. GitHub Integration via API & Actions
The detector will integrate directly with GitHub using the GitHub REST API and GitHub Actions,
enabling automated scanning of pull requests and commits during the review process — no manual
...
real-time detection report for every contribution — without disrupting the development process.

---

### Core Idea

> **"Don't just look at the code — look at how it was written."**

The central innovation is shifting detection from **what the code does** to **how it behaves
as a contribution**. AI-generated code is not just syntactically different — it leaves
...
#### 🧠 2. AI vs. Human Classification Model
...
#### 📊 3. Confidence Score & Explainability Report
Every scan returns a **confidence score (0–100%)** indicating the likelihood of AI
generation, along with a breakdown of the contributing signals — such as token entropy
spikes, uniform naming patterns, commit size anomalies, and style inconsistencies — so
reviewers understand *why* a contribution was flagged.

#### 👤 4. Author Behavioral Profiling
Builds a baseline coding profile for each GitHub user by analyzing their historical
contributions — commit frequency, code style, complexity trends — and flags deviations
that suggest AI assistance, adding a personalized layer of context to every detection.

#### ⚙️ 5. GitHub Actions Integration
...
or PR URL, and instantly receive a detailed detection report — no setup required. Includes
...
Python, JavaScript, TypeScript, Java, C++, Go, and Rust — with plans to expand coverage
based on usage data.

#### 🔒 10. Privacy-First Design
No source code is stored permanently. All scans are processed ephemerally, and only
anonymized feature vectors are retained for model improvement — ensuring contributor
